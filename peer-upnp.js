 ...
 *
 * Modified by: Matt Brain (matt.brain@gmail.com) to support UPNP Alarm
...
RemoteDevice.prototype.bind = function(callback){
	var self = this;
	// Matt
	//console.log("Trying to get " + this.descriptionUrl);
...
var registerHTTPHandler = function(peer){
	peer.server && peer.server.on("request", peer.httpHandler = function(req,rsp){
		...
		if (isPeer) {
			// Matt added stuff below
			var path = url.pathname.substr(peer.prefix.length);
			//console.log ("->registerHTTPHandler "+ path)
...
//. Matt
//	console.log("HTTPRequest Option:"+ options +" body: " + body + " callback:"+ callback);
